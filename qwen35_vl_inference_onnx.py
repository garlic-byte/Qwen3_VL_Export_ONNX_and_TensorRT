...
class QwenVLDecoder:
    ...
    def generate(self,
                 ...
                 mm_token_type_ids: np.ndarray = None,
        ...
        Args:
            ...
            mm_token_type_ids: Token type ids matching each modality to a different value in the input sequence, i.e. text (0), image (1), video (2).
        ...
        for step in range(max_new_tokens):
            ...
            inputs = self._prepare_inputs(current_input_ids, current_attention_mask, pixel_values, image_grid_thw, mm_token_type_ids)
...
def chat_inputs(processor, mode):
    ...
    inputs = {
        ...
        "mm_token_type_ids": tensor_inputs["mm_token_type_ids"].cpu().numpy()
    }
...
def main(config: ArgsConfig):
    ...
    if config.inference_mode == "pytorch":
        torch_model = Qwen3_5ForConditionalGeneration.from_pretrained(config.qwen_path, dtype=torch.float32, device_map='cuda')
