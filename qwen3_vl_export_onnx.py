...
# Set the folder path where the Qwen3-VL whole project downloaded.
path = '/home/wsj/Desktop/data/Downloads/weights/qwen3-vl-2b'

# For QwenVL
onnx_model_A = '/home/wsj/Desktop/code/github/Isaac-GR00T/deployment_scripts/my_deploy/Qwen_ONNX/QwenVL_A.onnx'  # Assign a path where the exported QwenVL model stored.
onnx_model_B = '/home/wsj/Desktop/code/github/Isaac-GR00T/deployment_scripts/my_deploy/Qwen_ONNX/QwenVL_B.onnx'
onnx_model_C = '/home/wsj/Desktop/code/github/Isaac-GR00T/deployment_scripts/my_deploy/Qwen_ONNX/QwenVL_C.onnx'
onnx_model_D = '/home/wsj/Desktop/code/github/Isaac-GR00T/deployment_scripts/my_deploy/Qwen_ONNX/QwenVL_D.onnx'
onnx_model_E = '/home/wsj/Desktop/code/github/Isaac-GR00T/deployment_scripts/my_deploy/Qwen_ONNX/QwenVL_E.onnx'

# You must assign a different folder path to avoid duplicate weight file names, which would cause loading failures.
onnx_model_F = '/home/wsj/Desktop/code/github/Isaac-GR00T/deployment_scripts/my_deploy/Qwen_ONNX_2/QwenVL_F.onnx'

# For Beam_Search & Greedy_Search
onnx_model_G = '/home/wsj/Desktop/code/github/Isaac-GR00T/deployment_scripts/my_deploy/Qwen_ONNX/Greedy_Search.onnx'
onnx_model_H = '/home/wsj/Desktop/code/github/Isaac-GR00T/deployment_scripts/my_deploy/Qwen_ONNX/First_Beam_Search.onnx'
onnx_model_I = '/home/wsj/Desktop/code/github/Isaac-GR00T/deployment_scripts/my_deploy/Qwen_ONNX/Second_Beam_Search.onnx'
onnx_model_J = '/home/wsj/Desktop/code/github/Isaac-GR00T/deployment_scripts/my_deploy/Qwen_ONNX/Reset_Penality.onnx'
...
class FIRST_BEAM_SEARCH(torch.nn.Module):
    ...
    def forward(self, *all_inputs):
        ...
        return *self.save_keys_values, top_beam_indices, save_id, repeat_penality, top_beam_prob.transpose(0,
                                                                                                           1), batch_indices, max_logits_idx
...
class SECOND_BEAM_SEARCH(torch.nn.Module):
    ...
    def forward(self, *all_inputs):
        ...
        return *self.save_keys_values, top_beam_indices, save_id, repeat_penality, top_beam_prob.unsqueeze(
            -1), max_logits_idx
...
class QwenVL_PartB(torch.nn.Module):
    ...
    def forward(self, pixel_values):
        ...
        vision_hidden_states = self.qwenvl.visual.patch_embed.proj(pixel_values).view(1, -1,
                                                                                      self.qwenvl.visual.patch_embed.embed_dim)
        if batch_size != 1:
            if DYNAMIC_IMAGE_SHAPE:
                ...
                self.pos_embeds = torch.cat([self.pos_embeds for _ in range(batch_size)],
                                            dim=1)  # For batch size != 1 & static shape inputs.
        ...
        for layer_num, blk in enumerate(self.qwenvl.visual.blocks):
            ...
            q, k, v = blk.attn.qkv(hidden_states_norm).reshape(-1, 3, self.num_heads, self.head_dim).permute(1, 2, 0,
                                                                                                             3).split(
                [1, 1, 1], dim=0)
            ...
            if layer_num in self.qwenvl.visual.deepstack_visual_indexes:
                deepstack_layer = self.qwenvl.visual.deepstack_merger_list[
                    self.qwenvl.visual.deepstack_visual_indexes.index(layer_num)]
        ...
        vision_hidden_states = self.qwenvl.visual.merger.linear_fc2(self.qwenvl.visual.merger.act_fn(
            self.qwenvl.visual.merger.linear_fc1(
                vision_hidden_states.view(1, -1, self.qwenvl.visual.merger.hidden_size))))
...
class QwenVL_PartC(torch.nn.Module):
    ...
    def forward(self, *all_inputs):
        ...
        concate_hidden_states = torch.cat([text_hidden_states[:, :self.prompt_head_len], vision_hidden_states,
                                           text_hidden_states[:, self.prompt_head_len:]], dim=1)
        zeros_B = self.zeros_B[:, :text_hidden_states.shape[1] - self.prompt_head_len].float()
        deepstack_features = [torch.cat([self.zeros_A, all_inputs[i], zeros_B], dim=1) for i in
                              range(self.deepstack_features_len)]
...
class QwenVL_PartD(torch.nn.Module):
    def __init__(self, qwenvl, width_factor, height_factor, prompt_head_len, max_seq_len):
        ...
        fill_tail_position = torch.arange(self.start_id, max_seq_len + self.start_id, dtype=torch.float32).repeat(3, 1,
                                                                                                                  1)
        ...
        freqs = Qwen3VLTextRotaryEmbedding.apply_interleaved_mrope(qwenvl.model.language_model.rotary_emb, freqs,
                                                                   qwenvl.model.language_model.rotary_emb.mrope_section)
...
class QwenVL_PartF(torch.nn.Module):
    def __init__(self, qwenvl, head_dim, num_heads, num_layers, num_key_value_heads, max_seq_len, prompt_head_len,
                 deepstack_features_len, height_factor, width_factor):
    ...
    def forward(self, *all_inputs):
        ...
        for i, layer in enumerate(self.qwenvl.model.language_model.layers):
            ...
            k = layer.self_attn.k_proj(hidden_states_norm).view(batch_size, -1, 1, self.num_key_value_heads,
                                                                self.head_dim)
            v = layer.self_attn.v_proj(hidden_states_norm).view(batch_size, -1, 1, self.num_key_value_heads,
            ...
            attn_out = layer.self_attn.o_proj(torch.matmul(attn, v).transpose(1, 2).contiguous().view(batch_size, -1,
...
with torch.inference_mode():
    model = Qwen3VLForConditionalGeneration.from_pretrained(path, dtype=torch.float32, device_map="cpu",
    ...
    pixel_values = torch.randint(low=0, high=255,
                                 size=[VISION_BATCH_SIZE, 1, 3, INPUT_IMAGE_SIZE[0], INPUT_IMAGE_SIZE[1]]).to(
        torch.uint8)
    ...
    model_F = QwenVL_PartF(model, head_dim, num_heads, num_layers, num_key_value_heads, MAX_SEQ_LENGTH, prompt_head_len,
                           deepstack_features_len, HEIGHT_FACTOR, WIDTH_FACTOR)
    del model
    gc.collect()
    all_inputs = []
...
ort_session_A = onnxruntime.InferenceSession(onnx_model_A, sess_options=session_opts,
                                             providers=ORT_Accelerate_Providers, provider_options=provider_options,
...
ort_session_F = onnxruntime.InferenceSession(onnx_model_F, sess_options=session_opts,
                                             providers=ORT_Accelerate_Providers, provider_options=provider_options,
                                             run_options=run_options)
...
penality_value = onnxruntime.OrtValue.ortvalue_from_numpy(np.array(REPEAT_PENALITY, dtype=model_dtype), device_type,
                                                          DEVICE_ID)
...
if USE_BEAM_SEARCH:
    penality_reset_count_beam_init = onnxruntime.OrtValue.ortvalue_from_numpy(np.zeros(BEAM_SIZE, dtype=np.int32),
...
if device_type != 'dml':
    init_past_keys_F = onnxruntime.OrtValue.ortvalue_from_numpy(
        np.zeros((1, ort_session_F._inputs_meta[0].shape[1], 1, ort_session_F._inputs_meta[0].shape[3], 0),
                 dtype=model_dtype), device_type, DEVICE_ID)
    init_past_values_F = onnxruntime.OrtValue.ortvalue_from_numpy(np.zeros(
        (1, ort_session_F._inputs_meta[num_layers].shape[1], 1, 0, ort_session_F._inputs_meta[num_layers].shape[4]),
        dtype=model_dtype), device_type, DEVICE_ID)
else:
    init_past_keys_F = onnxruntime.OrtValue.ortvalue_from_numpy(
        np.zeros((1, ort_session_F._inputs_meta[0].shape[1], 1, ort_session_F._inputs_meta[0].shape[3], 0),
                 dtype=model_dtype), 'cpu', 0)
    init_past_values_F = onnxruntime.OrtValue.ortvalue_from_numpy(np.zeros(
        (1, ort_session_F._inputs_meta[num_layers].shape[1], 1, 0, ort_session_F._inputs_meta[num_layers].shape[4]),
        dtype=model_dtype), 'cpu', 0)
init_deepstack_features = onnxruntime.OrtValue.ortvalue_from_numpy(
    np.zeros((1, 1, ort_session_C._inputs_meta[0].shape[2]), dtype=model_dtype), device_type, DEVICE_ID)
init_save_id = onnxruntime.OrtValue.ortvalue_from_numpy(np.zeros((BEAM_SIZE, 0), dtype=np.int32), device_type,
                                                        DEVICE_ID)
init_repeat_penality = onnxruntime.OrtValue.ortvalue_from_numpy(np.ones((BEAM_SIZE, vocab_size), dtype=model_dtype),
                                                                device_type, DEVICE_ID)
init_batch_size_greedy = onnxruntime.OrtValue.ortvalue_from_numpy(np.array([1], dtype=np.int64), device_type, DEVICE_ID)
init_beam_size = onnxruntime.OrtValue.ortvalue_from_numpy(np.zeros((BEAM_SIZE, 0), dtype=np.int32), device_type,
                                                          DEVICE_ID)
